...
- Keep the locked Story redesign contract in `architecture/storyArchitecture.mjs` and `docs/STORY-ARCHITECTURE.md`. `npm run audit:story` enforces the nine-destination taxonomy, clean target routes, official/editorial arc distinction, hybrid Black Archive design, Yorknew prototype, Succession depth, manga/anime policy, and deferred mobile scope before later batches change the runtime.
