...
export const implementationSections = [
  ...
  },
  {
    ...
    decisions: [['Primitive first', 'Use ArchiveUI before inventing a new badge, source block, card, status pill, or ledger.'], ['Evidence vocabulary', 'EvidenceBadge mirrors the governance evidence-state language.'], ['Live usage', 'The home page uses the primitives so the library is not dead code.']],
  ...
  },
  {
    ...
    decisions: [['Lazy workspaces', 'Seventeen UI boundaries, two Story pages, and three search shards remain dynamic.'], ['Intent prefetch', 'Routes load after focus, pointer intent, or navigation.'], ['Deferred media', 'Only first-viewport portraits are eager.']],
    checks: [`Entry ${formatPerformanceBudget(performanceBudgets.entryJs)} bytes, startup JS ${formatPerformanceBudget(performanceBudgets.startupJs)} bytes, startup CSS ${formatPerformanceBudget(performanceBudgets.startupCss)} bytes, and JS chunk ${formatPerformanceBudget(performanceBudgets.javascriptChunk)} bytes remain canonical budgets.`, 'Twenty-two dynamic entries remain separated.', 'No PWA or service worker is introduced.'],
  },
  {
