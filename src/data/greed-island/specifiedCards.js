 ...
 * Verified: 2026-07-21
 *
 * This foundation intentionally registers no remote or local artwork. Visuals
 * remain pending until the URL and its local WebP fallback are both verified.
 */
...
function buildSpecifiedCards(records) {
  ...
  return records.map(([number, name, rank, conversionLimit], index) => {
    ...
    return Object.freeze({
      ...
      verification: Object.freeze({ core: 'verified', description: 'pending', acquisition: 'pending', story: 'pending', media: 'pending' }),
      media: Object.freeze({ remote: null, local: null, fallback: 'generated-card-back' }),
    });
  });
}
