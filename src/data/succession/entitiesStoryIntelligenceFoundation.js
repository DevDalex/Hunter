...
for (const research of successionChapterResearch) {
  if (research.number > lastDocumentedChapter || !research?.coverage?.chronology || !research.events?.length) continue;
  const ids = [];
...
const chapters = Object.freeze(nenFoundationData.chapters.map((chapter) => {
  ...
  const documentedResearch = chapter.number <= lastDocumentedChapter ? research : null;
  ...
  const outgoingCausalLinkIds = causalLinkValues
    ...
    .map((link) => link.id);
  const researchCharacterIds = resolveResearchCharacters(documentedResearch);
  ...
  const researchLocationIds = resolveResearchLocations(documentedResearch);
  const maintainedChapterEvents = maintainedEvents.filter((event) => event.chapterRange.start === chapter.number);
  return Object.freeze({
    ...chapter,
    name: documentedResearch?.title ? `Chapter ${chapter.number} · ${documentedResearch.title}` : chapter.name,
    summary: documentedResearch?.focus || chapter.summary,
    voyageDay: documentedResearch?.voyageDay || chapter.voyageDay,
    lanes: Object.freeze([...(documentedResearch?.lanes || chapter.lanes || [])]),
    ...
    updatedAt: documentedResearch?.lastReviewed || ARCHIVE_DATE,
  });
}));
