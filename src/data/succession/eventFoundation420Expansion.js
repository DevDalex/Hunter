import { succession420TimelineEvents } from '../succession420Research.js';
...
const abilityIds = (record) => /Parallel Future|parallel-future|Laplace|future-sight|perception|Zetsu/i.test(`${record.title} ${record.detail} ${(record.tracks||[]).join(' ')}`) ? freeze(['ability:parallel-future']) : freeze([]);
...
const criticalTracks = new Set(['parallel-future','combat','escape','chapter-endpoint','martial-law','hisoka','predicted-death','nen-perception','lockdown']);

export const eventFoundation420Expansion = freeze(succession420TimelineEvents.map((record,index)=>{
  const slug=`chapter420-${record.id.replace(/^420-/,'')}`;
  return freeze({
    ...
    canonLevel:record.confidence?.includes('speculation') || record.confidence?.includes('inference') || record.confidence?.includes('unresolved') ? 'inference' : 'canon',
    createdAt:'2026-09-09',updatedAt:'2026-09-09',category:record.tracks?.[0]||'chapter-420',
    importance:(record.tracks||[]).some((track)=>criticalTracks.has(track))?'critical':'major',
    chapterRange:freeze({start:420,end:420}),
    chronology:freeze({sequence:index+1,day:'Voyage Day 12',timeOfDay:record.time||null,storyPeriod:'Chapter 420 continuation',certainty:'chapter-presentation-order-confirmed'}),
