...
function GreedIslandHero({ onNavigate, mode, setMode, summoned, setSummoned }) {
  return <header className="gi-hero" id="entry">
    ...
    <div className="gi-hero__copy">
      ...
      <p>Learn the rules from Eta. Summon your Book. Examine cards, locations, quests, player targeting, and Game Master systems through source-linked archive tools.</p>
      <dl>
        ...
        <div><dt>Quest records</dt><dd>{greedIslandSystemStats.quests}</dd></div>
      </dl>
...
function FoundationNotes() {
  return <section className="gi-foundation" id="foundation" aria-labelledby="gi-foundation-title">
    <header className="gi-section-heading">
      ...
      <h2 id="gi-foundation-title">Cards, places, quests, players, and Game Masters now stay in linked systems.</h2>
      <p>The Specified archive, verified scans, reconstructed Book, Eta course, card libraries, island map, quest directory, player targeting, and Game Master room are connected without pretending to reproduce a live game state.</p>
    </header>
    <div className="gi-foundation__grid">
      <article><ShieldCheck size={23} /><span>Completed now</span><h3>Map and quest systems</h3><p>{greedIslandSystemStats.locations} verified location/facility records and {greedIslandSystemStats.quests} quest or acquisition records connect card goals to source-linked island places.</p></article>
      <article><MousePointer2 size={23} /><span>Usable now</span><h3>Player Binder logic</h3><p>Met-player targeting, scanning, attack risk, travel routing, and post-clear routing are demonstrated as archive simulations over documented card behavior.</p></article>
      <article><LockKeyhole size={23} /><span>Clearly labelled</span><h3>Game Master controls</h3><p>Eta, Elena, Razor, List/Dwun, and negative-number card controls are separated from player-accessible cards and marked as restricted where appropriate.</p></article>
    </div>
    <div className="gi-stage-list">
      <h3>Next verified stages</h3>
      <ol>{nextStages.map(([title, note], index) => <li key={title}><i>{String(index + 8).padStart(2, '0')}</i><div><strong>{title}</strong><p>{note}</p></div></li>)}</ol>
    </div>
  </section>;
}
...
export default function GreedIslandPage({ onNavigate }) {
  ...
  return <article className="greed-island-page">
    ...
    <nav className="gi-local-nav" aria-label="Greed Island page sections">
      <div><a href="#entry">Entry</a><a href="#tutorial">Eta tutorial</a><a href="#binder">Binder</a><a href="#card-archive">Specified archive</a><a href="#card-libraries">Card libraries</a><a href="#island-systems">Island systems</a><a href="#foundation">Build stages</a><a href="#sources">Sources</a></div>
    </nav>
    <main className="gi-canvas">
      ...
      {summoned ? <GreedIslandBinder /> : <section className="gi-book-gate" aria-labelledby="gi-book-gate-title">
        ...
        <p>The complete Eta tutorial, card archives, and island systems remain available. Story Mode leaves the Book absent until the player invokes the command; Free Exploration summons it immediately.</p>
      ...
      <section className="gi-sources" id="sources" aria-labelledby="gi-sources-title">
        <header className="gi-section-heading"><span>Primary research sources</span><h2 id="gi-sources-title">Hunterpedia / Hunter × Hunter Fandom</h2><p>Card images, effects, Book references, tutorial rules, acquisition records, story mappings, Spell classes, Free Slot examples, Game Master cards, locations, quests, and island control systems retain explicit sources and verification states.</p></header>
