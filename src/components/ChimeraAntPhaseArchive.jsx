...
function PhaseStateLedger({ phase }) {
  return (
    <dl className="chimera-phase-spread__states">
    ...
    <figure className="chimera-phase-spread__media">
      ...
      <figcaption>
        ...
        {detail?.media?.sourceHref ? (
          <a href={detail.media.sourceHref} target="_blank" rel="noreferrer noopener">
            {detail.media.creditLabel || 'Image source'} <ExternalLink size={12} />
          </a>
        ) : (
...
function EpisodeGroups({ groups = [] }) {
  return (
    <ol className="chimera-phase-spread__episode-groups" aria-label="Episode groups in this phase">
      {groups.map((group, index) => (
        <li key={`${group.range[0]}-${group.range[1]}`}>
...
export default function ChimeraAntPhaseArchive({ phases, activePhase, onSelectPhase, onNavigate, artwork, fallbackArtwork }) {
  return (
    <div className="chimera-phase-archive">
        ...
        return (
          ...
          >
            ...
            <footer className="chimera-phase-spread__footer">
              ...
              {nextPhase ? (
                <button type="button" onClick={() => onSelectPhase(nextPhase.id)}>
                  Continue to Phase {String(nextPhase.number).padStart(2, '0')} <ArrowRight size={14} />
                </button>
              ) : (
                <button type="button" onClick={() => document.getElementById('chimera-characters')?.scrollIntoView({ behavior: 'smooth', block: 'start' })}>
                  Continue to character records <ArrowRight size={14} />
                </button>
              )}
            </footer>
          </article>
