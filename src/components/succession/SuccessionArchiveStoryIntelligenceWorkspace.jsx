...
function PhaseDossier({ dossier, onBack, onNavigate }) {
  ...
  return <article className="succession-story-intel__dossier">
    ...
    <header><span>Story phase · Chapters {dossier.profile.chapterRange.start}–{dossier.profile.chapterRange.end}</span><h2>{dossier.profile.name}</h2><p>{dossier.profile.summary}</p><dl><div><dt>Visible chapters</dt><dd>{dossier.chapters.length}</dd></div><div><dt>Events</dt><dd>{dossier.events.length}</dd></div><div><dt>Threads</dt><dd>{dossier.threads.length}</dd></div></dl></header>
    <section><div className="succession-story-intel__section-title"><Layers3 size={18} /><div><span>Parallel structure</span><h3>Lanes active in this phase</h3></div></div><div className="succession-story-intel__link-grid">{dossier.lanes.map((lane) => <button type="button" key={lane.id} onClick={() => onNavigate('story', { lane: shortId(lane.id) })}><b>{lane.name}</b><span>{lane.objective}</span></button>)}</div></section>
    <section><div className="succession-story-intel__section-title"><Activity size={18} /><div><span>Chronology</span><h3>Events intersecting this phase</h3></div></div><div className="succession-story-intel__event-list">{dossier.events.map((event) => <article key={event.id}><span>Ch. {event.chapterRange.start}{event.chapterRange.end !== event.chapterRange.start ? `–${event.chapterRange.end}` : ''}</span><h4>{event.name}</h4><p>{event.summary}</p><button type="button" onClick={() => onNavigate('events', { entity: event.id })}>Open event</button></article>)}</div></section>
...
function LaneDossier({ dossier, onBack, onNavigate }) {
  ...
  return <article className="succession-story-intel__dossier">
    <StoryBack onBack={onBack} />
    <header><span>Parallel story lane · From Chapter {dossier.profile.chapterRange.start}</span><h2>{dossier.profile.name}</h2><p>{dossier.profile.summary}</p><blockquote>{dossier.profile.objective}</blockquote><dl><div><dt>Phases</dt><dd>{dossier.phases.length}</dd></div><div><dt>Events</dt><dd>{dossier.events.length}</dd></div><div><dt>Threads</dt><dd>{dossier.threads.length}</dd></div></dl></header>
    <section><div className="succession-story-intel__section-title"><Layers3 size={18} /><div><span>Arc movement</span><h3>Phase history</h3></div></div><div className="succession-story-intel__phase-strip">{dossier.phases.map((phase) => <button type="button" key={phase.id} onClick={() => onNavigate('story', { phase: shortId(phase.id) })}><span>{phase.chapterRange.start}–{phase.chapterRange.end}</span><b>{phase.name}</b></button>)}</div></section>
...
function ThreadDossier({ dossier, onBack, onNavigate }) {
  ...
  return <article className="succession-story-intel__dossier is-thread">
    <StoryBack onBack={onBack} />
    <header><span>{labelize(dossier.status)} · {labelize(profile.category)} · From Chapter {profile.chapterRange.start}</span><h2>{profile.name}</h2><p>{profile.question}</p><blockquote>{profile.evidenceState}</blockquote></header>
    {!!dossier.lanes.length && <section><div className="succession-story-intel__section-title"><Layers3 size={18} /><div><span>Story placement</span><h3>Connected plotlines</h3></div></div><div className="succession-story-intel__link-grid">{dossier.lanes.map((lane) => <button type="button" key={lane.id} onClick={() => onNavigate('story', { lane: shortId(lane.id) })}><b>{lane.name}</b><span>{lane.summary}</span></button>)}</div></section>}
...
export default function SuccessionArchiveStoryIntelligenceWorkspace({ routeParams = {}, spoilerLimit = 414, onNavigate }) {
  ...
  const phases = Object.values(successionArchiveData.storyPhaseProfiles || {}).filter((phase) => phase.chapterRange.start <= spoilerLimit);
  ...
  return <div className="succession-story-intel">
    ...
    <section className="succession-story-intel__current"><div><span>{snapshot.phase?.name || 'Pending phase'} · Chapter {snapshot.chapter}</span><h2>{snapshot.phase?.summary || 'Detailed maintained story annotation is pending.'}</h2></div><dl><div><dt>Events visible</dt><dd>{snapshot.counts.events}</dd></div><div><dt>New here</dt><dd>{snapshot.counts.startingEvents}</dd></div><div><dt>Named appearances</dt><dd>{snapshot.counts.appearances}</dd></div></dl></section>

    <div className="succession-story-intel__search"><label><Search size={16} /><span className="sr-only">Search story intelligence</span><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Phase, plotline, question, chapter focus…" /></label>{query && <div>{searchResults.map((result) => <button type="button" key={`${result.kind}:${result.id}`} onClick={() => openSearchResult(result)}><span>{result.kind}</span><b>{result.record.name}</b><small>{result.record.question || result.record.summary}</small></button>)}{!searchResults.length && <p>No story record is available inside Chapter {spoilerLimit}.</p>}</div>}</div>

    <section className="succession-story-intel__phases"><div className="succession-story-intel__section-title"><Layers3 size={19} /><div><span>Chronological architecture</span><h3>Contiguous phases from Chapter 340 to the current imported release</h3></div></div><div>{phases.map((phase) => <button type="button" className={snapshot.phase?.id === phase.id ? 'is-current' : ''} key={phase.id} onClick={() => onNavigate('story', { phase: shortId(phase.id) })}><span>{phase.chapterRange.start}–{phase.chapterRange.end}</span><h4>{phase.name}</h4><p>{phase.summary}</p><footer>{phase.laneIds.length} lanes · {phase.threadIds.length} emphasized threads <ArrowRight size={13} /></footer></button>)}</div></section>

    <section className="succession-story-intel__lanes"><div className="succession-story-intel__section-title"><Waypoints size={19} /><div><span>Parallel narrative</span><h3>{snapshot.lanes.length} plotlines active at Chapter {snapshot.chapter}</h3></div></div><div>{snapshot.lanes.map((lane) => <button type="button" key={lane.id} onClick={() => onNavigate('story', { lane: shortId(lane.id) })}><span>From Ch. {lane.chapterRange.start}</span><h4>{lane.name}</h4><p>{lane.summary}</p><b>{lane.objective}</b></button>)}</div></section>
