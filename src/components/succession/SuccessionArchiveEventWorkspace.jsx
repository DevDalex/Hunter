...
export default function SuccessionArchiveEventWorkspace({ routeParams = {}, spoilerLimit = 414, onNavigate }) {
  ...
  const ongoingCount = eligible.filter((event) => event.status === 'active-at-selected-chapter' || event.canonicalEvent.status === 'ongoing').length;
  const unresolvedCount = eligible.filter((event) => (event.openQuestions || []).length > 0 || !event.mature).length;
  ...
  return <div className="succession-canonical-events">
    <section className="succession-canonical-events__hero">
      ...
      <dl><div><dt>Visible events</dt><dd>{eligible.length}</dd></div><div><dt>Active</dt><dd>{ongoingCount}</dd></div><div><dt>Unresolved</dt><dd>{unresolvedCount}</dd></div><div><dt>Knowledge boundary</dt><dd>Ch. {knowledgeBoundary}</dd></div></dl>
    </section>
    ...
    <section className="succession-event-filter-panel" aria-labelledby="succession-event-filter-title">
      ...
      <footer><span role="status" aria-live="polite">Showing {visible.length} of {eligible.length} events through Chapter {knowledgeBoundary}.</span><button type="button" disabled={!activeFilterCount} onClick={resetFilters}>Clear filters</button></footer>
    </section>
    ...
    <section className="succession-canonical-event-timeline" aria-label="Canonical succession event timeline">
      ...
      {!visible.length && <div className="succession-canonical-event-timeline__empty"><AlertTriangle size={20} aria-hidden="true" /><h3>No events match these filters</h3><p>Clear one or more facets or move the chapter boundary forward.</p><button type="button" onClick={resetFilters}>Reset event filters</button></div>}
    </section>
    ...
    {selected && dossier && <article className="succession-canonical-event-dossier" aria-labelledby="succession-event-dossier-title">
      <header>
        ...
        <div><span>{titleCase(selected.category)} · {titleCase(selected.status)} · {selected.knowledgeState}</span><h2 id="succession-event-dossier-title">{selected.name}</h2><p>{selected.summary}</p></div>
      ...
      <section className="succession-event-causality" aria-labelledby="succession-event-causality-title">
        <header><Network size={17} aria-hidden="true" /><div><span>Causal record</span><h3 id="succession-event-causality-title">Why it happened and what is known</h3></div></header>
        <div><section><span>Causes</span><ol>{selected.causes.map((item) => <li key={item}>{item}</li>)}</ol>{!selected.causes.length && <p>No canonical cause is recorded yet.</p>}</section><section className="is-action"><span>Event action</span><p>{selected.summary}</p></section><section><span>Available outcomes</span><ol>{selected.outcomes.map((item) => <li key={item}>{item}</li>)}</ol>{!selected.outcomes.length && <p>Later outcomes remain hidden or undocumented at Chapter {knowledgeBoundary}.</p>}</section></div>
      </section>

      {!!selected.stateChanges.length && <section className="succession-event-state-changes"><header><CircleDot size={16} aria-hidden="true" /><div><span>Temporal model</span><h3>Available state changes</h3></div></header><ol>{selected.stateChanges.map((item) => <li key={item}>{item}</li>)}</ol></section>}
      ...
      <section className="succession-event-chain" aria-labelledby="succession-event-chain-title"><header><GitBranch size={17} aria-hidden="true" /><div><span>Event graph</span><h3 id="succession-event-chain-title">Available predecessors and consequences</h3></div></header><div><section><span>Leads into this event</span>{dossier.predecessors.length ? dossier.predecessors.map((event) => <button type="button" onClick={() => openEvent(eligible.find((candidate) => candidate.id === event.id) || event)} key={event.id}><small>Ch. {event.chapterRange?.start}</small><b>{event.name}</b><ArrowRight size={13} aria-hidden="true" /></button>) : <p>No predecessor is available at this boundary.</p>}</section><section><span>This event leads to</span>{dossier.consequences.length ? dossier.consequences.map((event) => <button type="button" onClick={() => openEvent(eligible.find((candidate) => candidate.id === event.id) || event)} key={event.id}><small>Ch. {event.chapterRange?.start}</small><b>{event.name}</b><ArrowRight size={13} aria-hidden="true" /></button>) : <p>No consequence event is available at this boundary.</p>}</section></div></section>

      {!!dossier.sources.length && <section className="succession-event-sources" aria-labelledby="succession-event-sources-title"><header><BookOpen size={17} aria-hidden="true" /><div><span>Evidence trail</span><h3 id="succession-event-sources-title">Chapter sources through {knowledgeBoundary}</h3></div></header><div>{dossier.sources.map((source) => <SourceReference source={source} onNavigate={onNavigate} key={source.id} />)}</div></section>}
