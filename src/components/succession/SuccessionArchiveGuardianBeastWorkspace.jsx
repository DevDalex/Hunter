...
function BeastCard({ record, mode, onOpen }) {
  ...
  return <button type="button" className={`succession-gsb-command-card is-${classification} is-host-${hostClass}`} onClick={() => onOpen(beast.id)}>
    <div className="succession-gsb-command-card__visual"><EntityVisual entity={mode === 'host' ? host : beast} /><span style={{ color: 'black' }}>{host?.princeOrder ? String(host.princeOrder).padStart(2, '0') : 'K'}</span></div>
