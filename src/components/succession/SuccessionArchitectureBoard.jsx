...
const preservedContracts = [
  'Deep links preserved', 'No duplicate Archive Home', 'Existing routes remain valid',
  'Records and IDs preserved', 'Reader accessed through Story', 'Accessibility and audits remain enforced',
];
...
function ViewCell({ item, onNavigate }) {
  ...
  return <a href={routeToHref('succession', item.target, item.params || {})} aria-label={item.label} className="succession-architecture__view-cell" onClick={(event) => { event.preventDefault(); onNavigate(item.target, item.params || {}); }}>
...
function NavigationRail({ onNavigate, id }) {
  return <nav id={id} className="succession-architecture__navigation" aria-label="Top-level Succession navigation"><h2>Top-level navigation</h2>{primaryNavigation.map((item) => {
    const Icon = item.icon;
    return <a key={item.id} href={routeToHref('succession', item.target, item.params || {})} className={item.id === 'story' ? 'is-current' : ''} aria-current={item.id === 'story' ? 'page' : undefined} onClick={(event) => { event.preventDefault(); onNavigate(item.target, item.params || {}); }}><Icon size={25} strokeWidth={1.35} aria-hidden="true" /><span>{item.label}</span></a>;
  })}</nav>;
}
...
function LibraryRail({ onNavigate }) {
  return <aside className="succession-architecture__library"><h2>Library tools</h2>{libraryTools.map((item) => {
    const Icon = item.icon;
    return <a href={routeToHref('succession', item.target)} onClick={(event) => { event.preventDefault(); onNavigate(item.target, {}); }} key={item.label}><span><Icon size={31} strokeWidth={1.25} aria-hidden="true" /></span><div><strong>{item.label}</strong><small>{item.note}</small></div></a>;
  })}<div className="succession-architecture__library-map"><MapSketch compact /></div></aside>;
}

function PreservedContracts() {
  return <section className="succession-architecture__contracts"><div className="succession-architecture__contract-sigil"><Scale size={34} strokeWidth={1.3} aria-hidden="true" /></div><div><h2>Preserved contracts <span>(system rules)</span></h2><ul>{preservedContracts.map((contract) => <li key={contract}>{contract}</li>)}</ul></div><div className="succession-architecture__integrity-seal"><LockKeyhole size={28} strokeWidth={1.3} aria-hidden="true" /><b>System integrity</b><small>Continuity first</small></div></section>;
}

function FooterSpecifications() {
  return <div className="succession-architecture__footer-specs"><section><h2>Legend</h2><ul><li><i className="is-solid" />Section</li><li><i />Sub-view</li><li><span>≡</span>Data set</li><li><GitBranch size={13} />Relation</li><li><ExternalLink size={13} />External link</li></ul></section><section><h2>Notes</h2><p>This architecture defines the information structure and navigation logic only.</p><p>Visual design, components, and micro-interactions will be defined in later phases.</p></section><section><h2>Reference grid</h2><div className="succession-architecture__reference-grid"><span>10px</span><b /><i /></div></section></div>;
}
...
function PageSkeleton({ onNavigate, onOpenSearch }) {
  return <section className="succession-architecture__skeleton-block"><div className="succession-architecture__callouts is-left"><p><b>Top header</b><span>Section header,<br />global utilities</span></p><p><b>Sidebar</b><span>Top-level navigation<br />persistent</span></p><p><b>Breadcrumb</b><span>Contextual path<br />within section</span></p><p><b>Timeline / map area</b><span>Temporal and spatial<br />context support</span></p></div>
    <div className="succession-architecture__page-skeleton"><header><strong>Succession Contest</strong><button type="button" className="succession-button" onClick={onOpenSearch}>Search archive… <Search size={13} /></button></header><div className="succession-architecture__skeleton-body"><nav aria-label="Skeleton navigation">{primaryNavigation.map((item) => { const Icon = item.icon; return <button type="button" aria-label={item.label} onClick={() => onNavigate(item.target, item.params || {})} key={item.id}><Icon size={18} strokeWidth={1.35} /></button>; })}</nav><main><ol><li>Home</li><li>Succession Contest</li><li>Story Intelligence</li><li>Timeline</li></ol><div className="succession-architecture__skeleton-tabs"><span>Story</span><span>Chapters</span><b>Timeline</b><span>Events</span></div><section><div className="succession-architecture__mock-content"><h3>Timeline: Key Moments</h3><div>{[1, 2, 3, 4].map((item) => <p key={item}><i /><span /></p>)}</div></div><MapSketch compact /></section><footer><div>{['1010', '1015', '1020', '1025', '1030', '1035'].map((year) => <span key={year}><i />{year}</span>)}</div><MapSketch compact /></footer></main><aside><h3>Related entity</h3><div><Users size={27} /><span /><span /></div><h3>Key data</h3>{[1, 2, 3, 4].map((item) => <p key={item}><i /><span /></p>)}</aside></div></div>
...
export default function SuccessionArchitectureBoard({ spoilerLimit, onNavigate, onExitArchive, onOpenSearch }) {
  ...
  return <article className="succession-archive succession-architecture-board" data-archive-route="story" data-archive-hub="story"><a className="succession-archive__skip-link" href="#succession-workspace-content">Skip to architecture</a><span className="sr-only" role="status" aria-live="polite">Succession Contest architecture board loaded. Reading boundary Chapter {spoilerLimit}.</span>
    ...
    <div id="succession-workspace-content" className="succession-archive__content" role="region" aria-label="Succession Contest architecture" tabIndex="-1"><div className="succession-architecture__sheet"><i className="succession-architecture__corner is-nw" aria-hidden="true" /><i className="succession-architecture__corner is-ne" aria-hidden="true" /><i className="succession-architecture__corner is-sw" aria-hidden="true" /><i className="succession-architecture__corner is-se" aria-hidden="true" />
      <header className="succession-architecture__masthead"><button type="button" className="succession-architecture__identity" onClick={onExitArchive}><span><Ship size={27} strokeWidth={1.25} aria-hidden="true" /></span><div><strong>Kakin Empire Archive</strong><small>Information architecture board<br />Classified — internal use only</small></div></button><div className="succession-architecture__title"><h1>Succession Contest</h1><p>Section architecture before final redesign</p></div><div className="succession-architecture__document-meta"><dl><div><dt>Architecture doc.</dt><dd>SC–IA–01</dd></div><div><dt>Date</dt><dd>08.01.2026</dd></div><div><dt>Version</dt><dd>0.9</dd></div></dl><span>Draft</span><div className="succession-architecture__stamp"><b>Dossier</b><small>SC–IA–01</small></div></div></header>
      ...
      <div className="succession-architecture__lower-grid"><div><PreservedContracts /><FooterSpecifications /></div><PageSkeleton onNavigate={onNavigate} onOpenSearch={onOpenSearch} /></div><footer className="succession-architecture__document-footer"><span>SC–IA–01</span><span>Reading boundary: Chapter {spoilerLimit}</span><span>Continuity first</span></footer>
    </div></div>
