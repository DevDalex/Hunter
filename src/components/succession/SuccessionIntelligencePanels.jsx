...
import { Check, CircleHelp, Clipboard, GitCompare } from 'lucide-react';
...
const routeByEntityType = {
  character: 'characters', organization: 'organizations', ability: 'nen', 'guardian-beast': 'guardian-spirit-beasts',
  event: 'events', chapter: 'chapters', location: 'locations', relationship: 'relationships', assignment: 'bodyguards',
};
...
const differs = (values) => JSON.stringify(values?.[0] ?? null) !== JSON.stringify(values?.[1] ?? null);

export default function SuccessionIntelligencePanels({ routeId = 'research', routeParams = {}, spoilerLimit, onNavigate }) {
  const [domain, setDomain] = useState(routeParams.compareDomain || 'characters');
  const [leftId, setLeftId] = useState(routeParams.compareLeft || '');
  const [rightId, setRightId] = useState(routeParams.compareRight || '');
  const [questionFilter, setQuestionFilter] = useState(routeParams.questionStatus || 'all');
  ...
  const updateComparisonRoute = (nextDomain, nextLeft, nextRight) => {
    onNavigate?.(routeId, {
      ...routeParams,
      compareDomain: nextDomain,
      ...(nextLeft ? { compareLeft: nextLeft } : {}),
      ...(nextRight ? { compareRight: nextRight } : {}),
    });
  };
    ...
    return domains.flatMap((entityType) => getEntitiesByType(entityType).flatMap((entity) => {
      ...
      return asArray(values).filter(Boolean).map((question, index) => {
        ...
        return {
          ...
          candidates: asArray(record.candidates), evidenceFor: asArray(record.evidenceFor || record.supportingEvidence),
  ...
  return <section className="succession-intelligence-panels" aria-label="Comparison and open questions">
    <article className="succession-intelligence-panels__compare">
      <header><GitCompare size={18} aria-hidden="true" /><div><span>Comparison engine</span><h2>Compare canonical records</h2></div>{comparison && <button type="button" onClick={shareComparison}>{copied ? <Check size={15} /> : <Clipboard size={15} />} {copied ? 'Copied' : 'Copy comparison link'}</button>}</header>
      <div className="succession-intelligence-panels__selectors">
        <label>Domain<select value={domain} onChange={(event) => { const value = event.target.value; setDomain(value); setLeftId(''); setRightId(''); updateComparisonRoute(value, '', ''); }}>{Object.keys(comparisonDomains).map((key) => <option key={key} value={key}>{key}</option>)}</select></label>
        <label>First record<select value={leftId} onChange={(event) => { const value = event.target.value; setLeftId(value); updateComparisonRoute(domain, value, rightId); }}><option value="">Choose…</option>{options.map((record) => <option key={record.id} value={record.id}>{labelOf(record)}</option>)}</select></label>
        <label>Second record<select value={rightId} onChange={(event) => { const value = event.target.value; setRightId(value); updateComparisonRoute(domain, leftId, value); }}><option value="">Choose…</option>{options.map((record) => <option key={record.id} value={record.id}>{labelOf(record)}</option>)}</select></label>
      </div>
      {comparison ? <div className="succession-intelligence-panels__table" role="table" aria-label={`${domain} comparison`}>
        ...
        {comparison.rows.map((row) => <div role="row" className={differs(row.values) ? 'is-different' : 'is-same'} key={row.field}><strong role="rowheader">{row.field}</strong><span role="cell">{printable(row.values[0])}</span><span role="cell">{printable(row.values[1])}</span></div>)}
      </div> : <p>Select two different records to generate a chapter-bounded comparison. The selection is written into the URL so it can be shared and reopened.</p>}
    </article>
    ...
    <article className="succession-intelligence-panels__questions">
      ...
      <label>Question status<select value={questionFilter} onChange={(event) => { const value = event.target.value; setQuestionFilter(value); onNavigate?.(routeId, { ...routeParams, questionStatus: value }); }}><option value="all">All</option>{statuses.map((status) => <option key={status} value={status}>{status}</option>)}</select></label>
      {visibleQuestions.length ? <div className="succession-intelligence-panels__question-list">{visibleQuestions.slice(0, 30).map((item) => <details key={item.id} id={`question-${item.id}`}>
        ...
        <button type="button" onClick={() => onNavigate?.(routeByEntityType[item.entityType] || 'research', { entity: item.entity.id, question: item.id })}>Open related dossier</button>
