...
const principlePoints = { Ten: [50, 21], Zetsu: [79, 50], Ren: [50, 79], Hatsu: [21, 50] };
const techniquePoints = { Gyo: [21, 9], In: [79, 9], En: [92, 35], Shu: [86, 76], Ko: [50, 91], Ken: [14, 76], Ryu: [8, 35] };
...
function NenPrincipleMap({ onOpenRecord }) {
  ...
  const chooseTechnique = (item) => {
    const nextPrinciple = item.bases.includes(activePrinciple) ? activePrinciple : (item.bases[0] || item.defaultPrinciple);
  ...
  return <section className="nen-principle-workbench" aria-labelledby="nen-principle-title">
    ...
    <div className="nen-principle-workbench__layout">
      <div className={`nen-principle-map is-stack-${activePrinciple.toLowerCase()}`} aria-label="Interactive relationship map of the Four Major Principles and advanced Nen techniques">
        ...
        <svg className="nen-principle-links" viewBox="0 0 100 100" preserveAspectRatio="none" aria-hidden="true">{related.map((item) => { const [targetX, targetY] = techniquePoints[item.name]; return <line x1={originX} y1={originY} x2={targetX} y2={targetY} key={`${activePrinciple}-${item.name}`} />; })}</svg>
        <div className="nen-principle-map__center"><span>念</span><small>Nen</small></div>
        {principles.map((item) => <button type="button" className={`nen-principle-node is-${item.position}${activePrinciple === item.name ? ' is-active' : ''}`} aria-pressed={activePrinciple === item.name} onMouseEnter={() => choosePrinciple(item.name)} onFocus={() => choosePrinciple(item.name)} onClick={() => choosePrinciple(item.name)} key={item.name}><i>{item.kanji}</i><span><strong>{item.name}</strong><small>{item.action}</small></span></button>)}
        {visibleTechniques.map((item) => { const connected = item.bases.includes(activePrinciple); const isChained = item.bases.length === 0; return <button type="button" className={`nen-advanced-node is-${item.position}${connected ? ' is-related' : ''}${isChained ? ' is-chained' : ''}${technique?.name === item.name ? ' is-active' : ''}`} aria-label={`${item.name}: ${item.base}${isChained ? '; chained technique, not a direct Four Major Principle application' : ''}`} onMouseEnter={() => chooseTechnique(item)} onFocus={() => chooseTechnique(item)} onClick={() => chooseTechnique(item)} key={item.name}><strong>{item.name}</strong><small>{item.base}</small></button>; })}
      </div>
      <aside className="nen-principle-inspector" aria-live="polite">
        <div className="nen-principle-inspector__foundation"><figure><SafeImage src={principle.image} fallbackLabel={principle.name} alt={`${principle.name} Nen concept diagram`} /></figure><span>Foundation · {principle.kanji}</span><h3>{principle.name}</h3><p>{principle.summary}</p><blockquote><b>Trade-off</b>{principle.risk}</blockquote></div>
        <div className="nen-principle-inspector__advanced"><header><span>{isChainedTechnique ? 'Chained advanced application' : `${related.length} direct application${related.length === 1 ? '' : 's'}`}</span><strong>{technique?.name}</strong></header>{technique && <figure><SafeImage src={technique.image} fallbackLabel={technique.name} alt={`${technique.name} Nen concept diagram`} /><figcaption>{technique.base}</figcaption></figure>}<p>{technique?.detail}</p>{isChainedTechnique && <small className="nen-principle-inspector__chain">Ryu is Gyo used while maintaining Ken. It is therefore shown as a second-stage technique, not as a direct branch of Ten or Ren.</small>}<div>{visibleTechniques.map((item) => <button type="button" className={technique?.name === item.name ? 'is-active' : ''} onMouseEnter={() => chooseTechnique(item)} onFocus={() => chooseTechnique(item)} onClick={() => chooseTechnique(item)} key={item.name}>{item.name}</button>)}</div><button type="button" className="nen-principle-inspector__records" onClick={() => onOpenRecord(technique?.name || activePrinciple, technique ? 'Advanced applications' : 'Four Major Principles')}>Open matching records <ArrowRight size={13} /></button></div>
      </aside>
    </div>
  </section>;
}
...
export default function NenEncyclopedia({ initialQuery = '', spoilerLimit = Number.MAX_SAFE_INTEGER }) {
  ...
  const visible = useMemo(() => {
    ...
    return nenRecords.filter((item) => (!item.chapter || item.chapter <= spoilerLimit)
  ...
  return <section className="nen-encyclopedia nen-image-desk" id="nen">
    ...
    {lessonView === 'foundations' && <section className="nen-foundation-view"><NenPrincipleMap onOpenRecord={openRecordSearch} /><div className="nen-foundation-reference"><div className="nen-foundation-index">{foundations.map((lesson, index) => <button className={selectedLesson.name === lesson.name ? 'is-active' : ''} onClick={() => setLessonIndex(index)} key={lesson.name}><i>{lesson.number}</i><span><strong>{lesson.name}</strong><small>{lesson.people.join(' · ')}</small></span></button>)}</div><article className="nen-evidence-card"><div className="nen-evidence-card__images">{selectedLesson.people.map((name) => <a href={portraitItem(name).source} target="_blank" rel="noreferrer" key={name}><Portrait name={name} /><span>{name}</span></a>)}</div><div className="nen-evidence-card__copy"><span>Lesson {selectedLesson.number} · sourced visual subjects</span><h3>{selectedLesson.name}</h3><p>{selectedLesson.summary}</p><blockquote><b>What to notice</b>{selectedLesson.look}</blockquote><a href={selectedLesson.source} target="_blank" rel="noreferrer">Open the exact Hunterpedia section <ExternalLink size={12} /></a></div></article></div></section>}
    ...
    <div className="nen-directory-heading"><div><span className="section-kicker">Reference directory</span><h3>Concepts and named abilities</h3></div><p>Use the complete map and focused lessons for orientation, then inspect mechanics, users, conditions, examples, and sources.</p><div><a href={nenSource} target="_blank" rel="noreferrer">Nen source <ExternalLink size={12} /></a><a href={nenAbilityDirectory} target="_blank" rel="noreferrer">Ability directory <ExternalLink size={12} /></a></div></div>
    <div className="nen-encyclopedia__toolbar"><label><Search size={16} /><input value={query} onChange={(event) => setQuery(event.target.value)} placeholder="Technique, user, category, condition…" /></label><div className="nen-record-type"><button className={recordType === 'all' ? 'is-active' : ''} onClick={() => setRecordType('all')}>All records</button><button className={recordType === 'concepts' ? 'is-active' : ''} onClick={() => setRecordType('concepts')}>System concepts</button><button className={recordType === 'abilities' ? 'is-active' : ''} onClick={() => setRecordType('abilities')}>Named abilities</button></div><div><button className={group === 'all' ? 'is-active' : ''} onClick={() => setGroup('all')}>All groups</button>{nenGroups.map((item) => <button className={group === item ? 'is-active' : ''} onClick={() => setGroup(item)} key={item}>{item}</button>)}</div><span role="status" aria-live="polite">{visible.length} records</span></div>
    <div className="nen-browser"><div className="nen-browser__index" role="listbox" aria-label="Nen records">{visible.map((item) => <button role="option" aria-selected={selected?.id === item.id} className={selected?.id === item.id ? 'is-active' : ''} onClick={() => setSelectedId(item.id)} key={item.id}><span>{item.group}</span><strong>{item.name}</strong><small>{item.user || item.kind}</small></button>)}{!visible.length && <div className="library-empty">No Nen record matches this search and spoiler boundary.</div>}</div>{selected && <article className="nen-record"><header><span>{selected.kind} · {selected.group}</span><h3>{selected.name}</h3><p>{selected.summary}</p></header>{selected.user && <div className="nen-record__user"><Portrait name={selected.user.split(',')[0].trim()} /><span><small>Primary indexed user</small><strong>{selected.user}</strong></span></div>}{(selected.user || selected.type || selected.debut) && <dl><div><dt>User</dt><dd>{selected.user || 'System concept'}</dd></div><div><dt>Type</dt><dd>{selected.type || 'Multiple / not applicable'}</dd></div><div><dt>Debut or scope</dt><dd>{selected.debut || 'Nen system record'}</dd></div></dl>}{/ability/i.test(selected.kind) && <section className="nen-ability-anatomy"><header><span>Ability anatomy</span><strong>Read the effect as a rule system</strong></header><ol><li><i>01</i><span><small>Effect</small><p>{selected.summary}</p></span></li><li><i>02</i><span><small>Activation / operation</small><p>{selected.mechanics[0] || 'Not separately established in the local record.'}</p></span></li><li><i>03</i><span><small>Conditions / persistence</small><p>{selected.mechanics.slice(1).join(' ') || 'Open the Hunterpedia source for currently established conditions.'}</p></span></li><li><i>04</i><span><small>Evidence boundary</small><p>{selected.study}</p></span></li></ol></section>}<section><h4>Mechanics</h4><ul>{selected.mechanics.map((item) => <li key={item}>{item}</li>)}</ul></section><section><h4>Study note</h4><p>{selected.study}</p></section><section><h4>Related records</h4><div>{selected.related.map((item) => <button onClick={() => { const match = nenRecords.find((record) => record.name === item); if (match) { setGroup('all'); setQuery(''); setSelectedId(match.id); } else setQuery(item); }} key={item}>{item}</button>)}</div></section><a href={selected.source} target="_blank" rel="noreferrer">Open Hunterpedia source <ExternalLink size={13} /></a></article>}</div>
  </section>;
}
