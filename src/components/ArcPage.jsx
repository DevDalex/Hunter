...
function CharacterGrid({ characters, onNavigate }) {
  return <div className="arc-character-grid">{characters.map((character) => <article key={character.name}>
    {character.image && !unavailablePortraits.has(character.name) && <figure><SafeImage src={character.image} alt={`${character.name} portrait from Hunterpedia`} /></figure>}
