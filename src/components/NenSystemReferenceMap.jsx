...
const PAN_STEP = 76;
const INSPECTOR_RECT = { x: 1900, y: 34, w: 304, h: 286 };

const categories = [
  { key: 'category:enhancement', name: 'Enhancement', code: 'En', mark: '強', cx: 1040, cy: 250, water: 'Water volume changes', summary: 'Strengthen the body, aura, objects, or an existing quality.', userPort: 'top', users: [['Gon Freecss', 'Jajanken', 840, 26], ['Uvogin', 'Big Bang Impact', 1050, 26]] },
  { key: 'category:transmutation', name: 'Transmutation', code: 'Tr', mark: '変', cx: 1320, cy: 420, water: 'Water taste changes', summary: 'Give aura the properties of another substance or phenomenon.', userPort: 'right', users: [['Killua Zoldyck', 'Godspeed', 1418, 332], ['Hisoka Morow', 'Bungee Gum', 1418, 424]] },
  { key: 'category:conjuration', name: 'Conjuration', code: 'Co', mark: '具', cx: 1320, cy: 700, water: 'Impurities appear', summary: 'Materialize an object, structure, creature, or rule-bearing construct.', userPort: 'right', users: [['Kurapika', 'Conjured chains', 1418, 654], ['Shizuku Murasaki', 'Blinky', 1418, 746]] },
  { key: 'category:specialization', name: 'Specialization', code: 'Sp', mark: '特', cx: 1040, cy: 870, water: 'A unique change occurs', summary: 'Produce an exceptional effect outside the regular five categories.', userPort: 'bottom', users: [['Chrollo Lucilfer', 'Skill Hunter', 840, 982], ['Neon Nostrade', 'Lovely Ghostwriter', 1050, 982]] },
  { key: 'category:manipulation', name: 'Manipulation', code: 'Ma', mark: '操', cx: 760, cy: 700, water: 'The leaf moves', summary: 'Control a person, object, creature, substance, or process.', userPort: 'left', users: [['Illumi Zoldyck', 'Needle People', 498, 654], ['Shalnark', 'Black Voice', 498, 746]] },
  { key: 'category:emission', name: 'Emission', code: 'Em', mark: '放', cx: 760, cy: 420, water: 'Water color changes', summary: 'Separate aura from the body while retaining its force or function.', userPort: 'left', users: [['Leorio Paradinight', 'Remote Punch', 498, 332], ['Razor', '14 Devils', 498, 424]] },
];
...
const foundationNodes = [
  ...
  { key: 'nen', kind: 'core', name: 'Nen', eyebrow: 'Aura control', x: 790, y: 40, w: 180, h: 88, summary: 'The discipline of sensing, retaining, producing, shaping, and applying aura.' },
...
const categoryNodes = categories.flatMap((category) => [
  { ...category, kind: 'category', x: category.cx - 80, y: category.cy - 80, w: 160, h: 160 },
  ...category.users.map(([name, ability, x, y]) => ({ key: `user:${name}`, kind: 'user', name, ability, category: category.name, categoryKey: category.key, x, y, w: 188, h: 82, portraitName: name })),
]);

const detailsNode = { key: 'terminal:details', kind: 'terminal', name: 'Details', x: 1814, y: 144, w: 74, h: 44, summary: 'The inspector previews and pins records from the active map path.' };
const rawNodes = [...foundationNodes, ...techniqueNodes, ...categoryNodes, ...classifierNodes, ...abilityNodes, ...ruleNodes, ...specialNodes, detailsNode, ...junctionNodes];
const ringPairs = categories.map((category, index) => [category.key, categories[(index + 1) % categories.length].key]);

const userEdges = categoryNodes.filter((node) => node.kind === 'user').map((node) => {
  const category = categories.find((item) => item.key === node.categoryKey);
  const ports = category.userPort === 'right' ? ['right', 'left'] : category.userPort === 'left' ? ['left', 'right'] : category.userPort === 'top' ? ['top', 'bottom'] : ['bottom', 'top'];
  return { id: `user:${node.categoryKey}:${node.key}`, from: node.categoryKey, to: node.key, type: 'user', fromPort: ports[0], toPort: ports[1] };
});
...
const graphEdges = [
  ...
  { id: 'hatsu-water', from: 'junction:hatsu-feed', to: 'concept:water-divination', type: 'classifier', points: [[1040, 144], [1559, 144]], toPort: 'top', label: 'classification', labelAt: [1320, 132] },
  { id: 'water-affinity', from: 'concept:water-divination', to: 'concept:category-affinity', type: 'classifier', fromPort: 'right', toPort: 'left' },
  { id: 'hatsu-developed', from: 'junction:hatsu-feed', to: 'ability:developed', type: 'ability', points: [[1040, 128], [1597, 128], [1597, 492]], toPort: 'top', label: 'developed ability', labelAt: [1340, 116] },
...
const port = (node, requested = 'center') => {
  if (!node || node.kind === 'junction') return { x: node?.x || 0, y: node?.y || 0 };
...
const computeBounds = (nodes, edges) => {
  const points = [];
  nodes.forEach((node) => {
    if (node.kind === 'junction') points.push([node.x, node.y]);
...
function MapNode({ node, record, active, pinned, onPreview, onClear, onPin, portraitItemFor }) {
  ...
  return <button type="button" className={`nen-pipe-node is-${node.kind}${active ? ' is-active' : ''}${pinned ? ' is-pinned' : ''}`} style={{ left: node.x, top: node.y, width: node.w, height: node.h }} aria-pressed={pinned} {...handlers}>
    ...
    <span className="nen-pipe-node__copy">{node.mark && <i>{node.mark}</i>}{node.eyebrow && <small>{node.eyebrow}</small>}<strong>{node.name}</strong>{node.code && <em>{node.code} · {node.water}</em>}{node.ability && <em>{node.ability}</em>}</span>
  </button>;
}
...
export default function NenSystemReferenceMap({ records = [], portraitItemFor }) {
  ...
  const inspectorNode = useMemo(() => ({ key: 'terminal:inspector', kind: 'junction', x: INSPECTOR_RECT.x, y: INSPECTOR_RECT.y + 142, w: 0, h: 0 }), []);
  ...
  const contentBounds = useMemo(() => computeBounds(rawNodes, graphEdges), []);
  ...
  const activeGraph = useMemo(() => {
    ...
    const addById = (id) => addEdge(graphEdges.find((edge) => edge.id === id));
    ...
    if (active.kind === 'user') { addEdge(graphEdges.find((edge) => edge.to === active.key && edge.type === 'user')); return { nodeKeys, edgeIds }; }
  ...
  const pointerMove = (event) => {
    ...
    if (gesture.mode === 'pinch' && gesture.pointers.size >= 2) {
      ...
      gesture.moved = true; setDragging(true); setView(boundedView({ scale, x: nextCenter.x - mapX * scale, y: nextCenter.y - mapY * scale }, viewportSize, contentBounds)); event.preventDefault();
      ...
      setView(boundedView({ ...gesture.startView, x: gesture.startView.x + dx, y: gesture.startView.y + dy }, viewportSize, contentBounds)); event.preventDefault();
    }
  };
  ...
  return <section className={`nen-pipe-map${active ? ' has-active' : ''}`} aria-label="Interactive Nen system pipeline map" onKeyDown={(event) => { if (event.key === 'Escape') { setPinned(null); setHovered(null); } }}>
      ...
      <div className="nen-pipe-canvas" style={{ width: MAP_WIDTH, height: MAP_HEIGHT, transform: `translate3d(${view.x}px,${view.y}px,0) scale(${view.scale})` }}>
        <svg className="nen-pipe-connectors" viewBox={`0 0 ${MAP_WIDTH} ${MAP_HEIGHT}`} fill="none" aria-hidden="true">
          {ringPairs.map(([fromKey, toKey]) => {
            const from = enrichedByKey.get(fromKey); const to = enrichedByKey.get(toKey); const a = center(from); const b = center(to); const activeRing = activeCategoryKey && (activeCategoryKey === fromKey || activeCategoryKey === toKey);
            return <g key={`${fromKey}-${toKey}`} className={activeRing ? 'is-active-ring' : ''}><path className="is-ring-under" d={`M ${a.x} ${a.y} L ${b.x} ${b.y}`} /><path className="is-ring" d={`M ${a.x} ${a.y} L ${b.x} ${b.y}`} />{[0.24, 0.5, 0.76].map((ratio, index) => <circle key={ratio} className={index === 1 ? 'is-halfway' : ''} cx={a.x + (b.x - a.x) * ratio} cy={a.y + (b.y - a.y) * ratio} r={index === 1 ? 8 : 6} />)}</g>;
          })}
          {graphEdges.map((edge) => {
            const from = enrichedByKey.get(edge.from); const to = enrichedByKey.get(edge.to); if (!from || !to) return null; const activeEdge = activeGraph.edgeIds.has(edge.id);
            return <g key={edge.id} className={`nen-edge-group${activeEdge ? ' is-active' : ''}`}><path className={`is-graph-edge is-${edge.type}${activeEdge ? ' is-active' : ''}`} d={edgePath(edge, from, to)} />{edge.label && edge.labelAt && <text className="nen-edge-label" x={edge.labelAt[0]} y={edge.labelAt[1]}>{edge.label}</text>}</g>;
          })}
          ...
          {activeCategoryKey && categories.map((category) => <g className="nen-efficiency-label" key={`efficiency:${category.key}`} transform={`translate(${category.cx},${category.cy - 98})`}><rect x="-25" y="-12" width="50" height="24" rx="12" /><text textAnchor="middle" dominantBaseline="central">{affinityLabel(activeCategoryKey, category.key)}</text></g>)}
        </svg>
