...
const typeRecords = [
  {
    name: 'Enhancement', code: 'En', mark: '強', slug: 'enhancement', position: 'top',
    summary: 'Strengthen the body, aura, objects, or an existing quality.',
    water: 'Water volume changes.', neighbors: ['Emission', 'Transmutation'],
    users: [['Gon Freecss', 'Jajanken'], ['Uvogin', 'Big Bang Impact']],
  },
  {
    name: 'Transmutation', code: 'Tr', mark: '変', slug: 'transmutation', position: 'upper-right',
    summary: 'Give aura the properties of another substance or phenomenon.',
    water: 'The water taste changes.', neighbors: ['Enhancement', 'Conjuration'],
    users: [['Killua Zoldyck', 'Godspeed'], ['Hisoka Morow', 'Bungee Gum']],
  },
  {
    name: 'Conjuration', code: 'Co', mark: '具', slug: 'conjuration', position: 'lower-right',
    summary: 'Materialize an object, structure, creature, or rule-bearing construct.',
    water: 'Impurities appear in the water.', neighbors: ['Transmutation', 'Specialization'],
    users: [['Kurapika', 'Conjured chains'], ['Shizuku Murasaki', 'Blinky']],
  },
  {
    name: 'Specialization', code: 'Sp', mark: '特', slug: 'specialization', position: 'bottom',
    summary: 'Produce an exceptional effect that does not fit the regular five categories.',
    water: 'A different or unique change occurs.', neighbors: ['Conjuration', 'Manipulation'],
    users: [['Chrollo Lucilfer', 'Skill Hunter'], ['Neon Nostrade', 'Lovely Ghostwriter']],
  },
  {
    name: 'Manipulation', code: 'Ma', mark: '操', slug: 'manipulation', position: 'lower-left',
    summary: 'Control a person, object, creature, substance, or process.',
    water: 'The leaf moves.', neighbors: ['Specialization', 'Emission'],
    users: [['Illumi Zoldyck', 'Needle People'], ['Shalnark', 'Black Voice']],
  },
  {
    name: 'Emission', code: 'Em', mark: '放', slug: 'emission', position: 'upper-left',
    summary: 'Separate aura from the body while retaining its force or function.',
    water: 'The water color changes.', neighbors: ['Manipulation', 'Enhancement'],
    users: [['Leorio Paradinight', 'Remote Punch'], ['Razor', '14 Devils']],
  },
];
...
function TypeNode({ item, active, onSelect, onOpenRecord }) {
  return <section className={`nen-reference-type is-${item.position}${active ? ' is-active' : ''}`} data-type={item.slug}>
    <button type="button" className="nen-reference-type__node" aria-pressed={active} onMouseEnter={() => onSelect(item.name)} onFocus={() => onSelect(item.name)} onClick={() => onSelect(item.name)}>
      <i>{item.mark}</i><strong>{item.code}</strong><span>{item.name}</span>
    </button>
    <div className="nen-reference-type__users">
      {item.users.map(([name, ability]) => <button type="button" onClick={() => onOpenRecord(name)} key={name}>
        <Portrait name={name} /><span><strong>{name}</strong><small>{ability}</small></span>
      </button>)}
    </div>
  </section>;
}

export default function NenSystemReferenceMap({ activeCategory = 'Enhancement', onSelectCategory, onOpenRecord }) {
  ...
  return <section className="nen-reference-sheet" aria-labelledby="nen-reference-title">
    <header className="nen-reference-sheet__title">
      <div><span>Fig. 01 · Hunter × Hunter reference project</span><h2 id="nen-reference-title">The complete<br />Nen system map</h2><p>The six categories and everything connected: aura control, affinities, advanced applications, conditions, curses, and the evidence needed to read an ability correctly.</p></div>
      <div className="nen-reference-sheet__key" aria-label="Affinity key"><strong>Affinity guide</strong><span><i className="is-red" /> Natural type · 100%</span><span><i /> Adjacent · 80%</span><span><i /> Distant · 60%</span><span><i /> Opposite · 40%</span></div>
    </header>
    ...
    <div className="nen-reference-sheet__body">
      ...
      <div className="nen-reference-map" aria-label="Interactive complete Nen category map">
        <svg className="nen-reference-map__lines" viewBox="0 0 100 100" aria-hidden="true">
          <polygon points="50,9 85,29 85,71 50,91 15,71 15,29" />
          <polygon points="50,25 71,37 71,63 50,75 29,63 29,37" />
          <line x1="50" y1="50" x2="50" y2="9" /><line x1="50" y1="50" x2="85" y2="29" /><line x1="50" y1="50" x2="85" y2="71" /><line x1="50" y1="50" x2="50" y2="91" /><line x1="50" y1="50" x2="15" y2="71" /><line x1="50" y1="50" x2="15" y2="29" />
        </svg>
        ...
        {typeRecords.map((item) => <TypeNode item={item} active={item.name === selected.name} onSelect={onSelectCategory} onOpenRecord={onOpenRecord} key={item.name} />)}
      </div>

      <aside className="nen-reference-column is-right">
        <section className="nen-reference-panel nen-reference-selected" data-type={selected.slug} aria-live="polite">
          <header><Droplets size={18} aria-hidden="true" /><h3>Water Divination</h3></header>
          <div><Portrait name={selected.users[0][0]} /><span><small>{selected.code} · {selected.name}</small><strong>{selected.water}</strong><p>{selected.summary}</p></span></div>
          <dl><div><dt>Adjacent</dt><dd>{selected.neighbors.join(' · ')}</dd></div><div><dt>Examples</dt><dd>{selected.users.map(([name]) => name).join(' · ')}</dd></div></dl>
          <button type="button" onClick={() => onOpenRecord(selected.name)}>Open {selected.name} records <ArrowRight size={13} /></button>
        </section>
