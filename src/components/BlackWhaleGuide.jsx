    ...
    <section className={`black-whale-section black-whale-section--atlas black-whale-intelligence is-${mapMode}`} id="black-whale">
      ...
      <div className="interactive-ship-atlas black-whale-intelligence__atlas">
        ...
        <aside className="ship-location-inspector" aria-live="polite">
          ...
          <dl className="ship-location-inspector__snapshot"><div><dt style={{ color: 'white' }}>Canonical record</dt><dd>{selectedCanonicalLocation?.name || selectedBridge.locationId}</dd></div><div><dt style={{ color: 'white' }}>Chapter</dt><dd>{snapshotChapter}</dd></div><div><dt style={{ color: 'white' }}>Occupants</dt><dd>{selectedCanonicalSnapshot?.occupants.length || 0}</dd></div><div><dt style={{ color: 'white' }}>Assignments</dt><dd>{selectedCanonicalSnapshot?.assignments.length || 0}</dd></div><div><dt style={{ color: 'white' }}>Events</dt><dd>{selectedCanonicalSnapshot?.events.length || 0}</dd></div><div><dt style={{ color: 'white' }}>Abilities</dt><dd>{selectedCanonicalSnapshot?.abilities.length || 0}</dd></div></dl>
      ...
      <section className="ship-manifest"><div className="subsection-title"><span>Passenger and staffing profile</span><h3>A city of 200,000</h3></div><div className="ship-manifest__table-wrap" role="region" aria-label="Black Whale passenger manifest" tabIndex="0"><table><thead><tr><th style={{ color: 'black' }}>Group</th><th style={{ color: 'black' }}>Count</th><th style={{ color: 'black' }}>Distribution</th><th style={{ color: 'black' }}>Operational meaning</th></tr></thead><tbody>{blackWhaleManifest.map((row) => <tr key={row.group}><th>{row.group}</th><td>{row.count}</td><td>{row.distribution}</td><td>{row.note}</td></tr>)}</tbody></table></div></section>
