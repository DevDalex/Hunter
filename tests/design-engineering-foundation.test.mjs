...
test('presentation records enforce spoiler and temporal boundaries', () => {
  ...
  const invalid = safeParseSuccessionPresentationRecord({ id: 'presentation:bad', spoilerFrom: 405, validFrom: 401 });
...
test('relationship graph exposes bounded multi-hop neighborhoods', () => {
  const graph = createSuccessionRelationshipGraph([
    { id: 'relationship:a-b', sourceEntityId: 'character:a', targetEntityId: 'character:b', direction: 'bidirectional', relationshipType: 'alliance', certainty: 'confirmed' },
    { id: 'relationship:b-c', sourceEntityId: 'character:b', targetEntityId: 'character:c', direction: 'directed', relationshipType: 'command', certainty: 'confirmed' },
  ]);
