...
assert(successionVisualPrinciples.some((item) => item.id === 'desktop-only' && item.rule.includes('1180px')), 'desktop-only foundation principle is missing');
assert(!successionVisualPrinciples.some((item) => item.id === 'responsive'), 'obsolete responsive foundation principle returned');
...
assert(css.includes('@media (prefers-reduced-motion: reduce)'), 'reduced-motion contract is required');
assert(!css.includes('@media (max-width:'), 'narrow-width foundation breakpoints must not return');
...
assert(bridgeCss.includes('.succession-evidence-workspace dt') && bridgeCss.includes('.succession-foundation-closure__domains span') && bridgeCss.includes('.succession-evidence-source-catalogue article > span'), 'Research labels below the readability floor must remain covered by the compatibility bridge');
assert(bridgeCss.includes('font-size: 11px'), 'Research compatibility labels must enforce an exact 11px floor');
...
assert(previewCss.includes('.succession-visual-preview'), 'preview CSS must remain scoped');
assert(!previewCss.includes('@media (max-width:'), 'development preview must not keep narrow-width presentation branches');
...
console.log(`Succession visual foundation audit passed: ${successionVisualPrinciples.length} principles, ${successionVisualTokenGroups.length} token groups, ${successionSemanticStates.length} semantic states, ${successionVisualComponentContracts.length} component contracts, scoped desktop-only CSS, semantic compatibility bridge, Research readability floor, hidden preview, reduced motion, inventory reporting, and issue #49 schedule documentation verified.`);
