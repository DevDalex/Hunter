...
assert(packageJson.scripts?.['foundation:static'] === 'npm run quality:foundation && npm run typecheck && npm run test:unit', 'foundation:static must compose linting, typing, and unit tests');
assert(packageJson.scripts?.['foundation:check'] === 'npm run foundation:static && npm run media:check', 'foundation:check must add generated-media verification to the static foundation gate');
assert(packageJson.scripts?.['media:check'] === 'npm run media:build && npm run media:verify', 'media:check must generate and verify canonical media variants');
assert(packageJson.scripts?.['build:runtime:prepared']?.includes('vite build') && packageJson.scripts['build:runtime:prepared'].includes('audit:release'), 'build:runtime:prepared must build and audit the Cloudflare artifact');
assert(!packageJson.scripts?.['build:runtime:prepared']?.includes('media:check'), 'prepared runtime build must not regenerate already-verified media');
assert(packageJson.scripts?.['build:runtime'] === 'npm run media:check && npm run build:runtime:prepared', 'standalone build:runtime must generate media before using the prepared runtime path');
assert(packageJson.scripts?.build === 'npm run check && npm run build:runtime:prepared', 'build must run the full check once and avoid duplicate media generation');
...
console.log(`Implementation notes audit passed: ${implementationSections.length} system sections; ${maintenanceMatrix.length} runbooks; ${releaseChecklist.reduce((total, group) => total + group.items.length, 0)} release checks; ${completionCriteria.length} completion criteria; split static/media foundation gate; single-pass production media generation; 15-audit aggregate preflight; split runtime/browser CI; pinned toolchain; build-only Sharp media processing; route, media, design-system, performance-budget, and Cloudflare contracts synchronized.`);
