...
assert(packageJson.scripts?.check === 'npm run test:unit && npm run generate:build-info && npm run preflight:build', 'check must run unit tests, generate build identity, and run aggregate preflight');
