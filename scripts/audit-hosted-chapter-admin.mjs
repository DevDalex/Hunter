...
const [engine, direct, serverIndex, directPage, inspectContract, workflow, prepareHosting, packageJson] = await Promise.all([
...
assert(directPage.includes('github.com/DevDalex/Hunter/issues/new') && directPage.includes('[Chapter Import]'), 'the page must create a prefilled GitHub import request');
...
assert(workflow.includes('import-succession-chapter-url.mjs') && workflow.includes('git push origin HEAD:main'), 'the workflow must run the existing importer and commit to main');
...
console.log('Hosted chapter importer audit passed: token-free inspection, visual preview, owner-only GitHub request workflow, built-in Actions commit permission, image parsing, manifest updates, and blocked legacy account/Worker publishing endpoints are intact.');
