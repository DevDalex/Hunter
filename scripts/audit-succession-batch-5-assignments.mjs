...
const assert = (condition, message) => { if (!condition) throw new Error(`Succession Batch 5 assignment audit failed: ${message}`); };
...
for (const token of ['succession-assignment-command','succession-assignment-command__metrics','succession-assignment-type-board','succession-assignment-result-command','succession-assignment-active-filters','AssignmentTable','AssignmentLedgerRow',"const [scope", "const [sortBy", "const [viewMode",'Complete archive','Active at Chapter','Show all active records','Command and obligation','Evidence record']) assert(workspace.includes(token), `assignment workspace is missing ${token}`);
for (const selector of ['.succession-assignment-command','.succession-assignment-command__signal','.succession-assignment-command__metrics','.succession-assignment-type-board','.succession-assignment-filter-panel','.succession-assignment-result-command','.succession-assignment-directory','.succession-assignment-table-wrap','.succession-assignment-table','.succession-assignment-ledger','.succession-assignment-dossier','.succession-assignment-snapshot-board','.succession-assignment-personnel']) assert(styles.includes(selector), `assignment visual system is missing ${selector}`);

assert(!styles.includes('@media (max-width:'), 'desktop-only assignment CSS must not carry narrow-width breakpoint layouts');
assert(!styles.includes('@media (hover: none)'), 'desktop-only assignment CSS must not carry no-hover device behavior');
assert(!styles.includes('(pointer: coarse)'), 'desktop-only assignment CSS must not carry coarse-pointer behavior');
assert(!styles.includes('touch-action:'), 'desktop-only assignment CSS must not carry touch-action rules');
...
console.log('Succession Batch 5 assignment audit passed: desktop operational command, snapshot scope, compound filters, sorting, cards, advanced table, compact ledger, pagination, personnel snapshots, dossiers, legacy cleanup, consolidated release QA, and reduced motion are registered.');
