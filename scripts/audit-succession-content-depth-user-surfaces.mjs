...
const [surfaceSource, entrySource, nenSource] = await Promise.all([
...
try {
  ...
  console.log(`Succession user-surface completion audit passed: ${chapter.fields.length} Chapter 417 fields, ${princes.length} prince dossiers, ${investigations.length} investigation families, ${nen.count} Nen records, ${glossary.count} glossary terms, and ${ledgers.length} operational ledgers are wired to normal site routes.`);
} finally {
  await vite.close();
}
