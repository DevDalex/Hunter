...
const assert = (condition, message) => {
  if (!condition) throw new Error(`Succession Batch 4 Nen audit failed: ${message}`);
};
...
assert(workflow.includes('node scripts/audit-succession-batch-4-nen.mjs'), 'Batch 4 workflow must run the Nen audit');
assert(workflow.includes('succession/nen'), 'Batch 4 workflow must render the Nen workspace');
for (const hour of ['Hour 47', 'Hour 48']) assert(docs.includes(hour), `Batch 4 design record must document ${hour}`);

console.log('Succession Batch 4 Nen audit passed: systems and abilities, knowledge-state filtering, owners and system facets, six-stage mechanic model, dedicated dossiers, chapter boundaries, responsive behavior, touch targets, and reduced motion are registered.');
