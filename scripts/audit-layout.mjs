...
assert(successionPages.length === 8 && referencePages.length === 6, 'the grouped workspaces must remain eight Succession and six encyclopedia screens');
...
assert(!app.includes("import SourceRegistry"), 'the removed Sources section is still mounted by the application');
...
console.log(`Layout audit passed: ${routeManifest.length} purposeful routes; connected family tree; contained media and tables; dominant ship atlas; responsive editorial shell.`);
