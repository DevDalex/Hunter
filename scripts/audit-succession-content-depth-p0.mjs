...
import { successionArchiveData } from '../src/data/succession/entitiesHighValueIntelligence.js';
import { validateHighValueIntelligenceData } from '../src/data/succession/highValueIntelligenceSchemas.js';
import { successionMysteryCases } from '../src/data/succession/successionMysteryCases.js';
import {
  contentDepthCurrentPhaseThreadIds417,
  contentDepthStoryThreads417,
} from '../src/data/succession/contentDepthStory417.js';
import {
  correctedStoryPhaseProfiles,
  correctedStoryThreadProfiles,
} from '../src/data/succession/storyIntelligenceCorrections.js';
...
const validation = validateHighValueIntelligenceData(successionArchiveData);
assert(validation.valid, `high-value intelligence validation failed: ${validation.errors.join(' | ')}`);
...
console.log(`Succession content depth P0 audit passed: ${validation.stats.knowledgeRecords} knowledge records, ${validation.stats.protocols} protocols, ${validation.stats.objects} objects, ${validation.stats.documents} documents, ${validation.stats.evidenceItems} evidence items, ${successionMysteryCases.length} mystery case files, and ${depthThreads.length} current-release Story threads through Chapter 417.`);
