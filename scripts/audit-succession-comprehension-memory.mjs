...
try {
  ...
  assert(state.watchlists[0].items.length === 1, 'research collection item was not retained');
  ...
  for (const token of ['Continue researching', 'Archive bookmarks', 'Compare tray', 'Saved searches', 'Research collections', 'Investigation note / working thesis', 'Export .md', 'local only']) assert(panel.includes(token), `Research Memory panel is missing ${token}`);
  ...
  assert(panel.includes('new Blob([markdownForWatchlist(watchlist)]') && panel.includes("type: 'text/markdown;charset=utf-8'") && panel.includes('anchor.download'), 'Markdown collection export is incomplete');
  assert(panel.includes('Notes and exports are personal research material, never canonical archive data.'), 'personal research material is not separated from canonical archive data');
  ...
  console.log(`Succession Research Memory audit passed: local recent/bookmark/search/compare/collection reducers, bounded notes, Markdown export, overview mount, and comparison/search handoffs are wired.`);
} finally {
  await vite.close();
}
