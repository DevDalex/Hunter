...
try {
  ...
  state = memory.withUpdatedArchiveBookmarkMetadata(state, item, { folder: 'Woble defense', tags: 'kurapika, room-1014, revisit' });
  assert(state.bookmarks[0].folder === 'Woble defense', 'bookmark folder was not retained');
  assert(state.bookmarks[0].tags.join('|') === 'kurapika|room-1014|revisit', 'bookmark tags were not normalized and retained');
  const cappedBookmarkTags = memory.withUpdatedArchiveBookmarkMetadata(state, item, { tags: Array.from({ length: 20 }, (_, index) => `bookmark-tag-${index}`), folder: 'x'.repeat(100) });
  assert(cappedBookmarkTags.bookmarks[0].tags.length === memory.SUCCESSION_ARCHIVE_BOOKMARK_TAG_LIMIT, 'bookmark tags exceeded their local boundary');
  assert(cappedBookmarkTags.bookmarks[0].folder.length === memory.SUCCESSION_ARCHIVE_BOOKMARK_FOLDER_LIMIT, 'bookmark folder exceeded its local text boundary');
  ...
  state = memory.withRenamedWatchlist(state, watchlistId, 'Room 1014 investigation', now);
  assert(state.watchlists[0].name === 'Room 1014 investigation' && state.watchlists[0].id === watchlistId, 'investigation rename did not preserve the stable collection ID');
  ...
  for (const token of ['Continue researching', 'Archive bookmark manager', 'Search bookmarks', 'All folders', 'Recently saved', 'Show all matching', 'Compare tray', 'Saved searches', 'Research collections', 'Investigation name', 'Rename', 'Investigation note / working thesis', 'Ordered evidence / record stack', 'Citations', 'Dossier .md', 'Print', 'local only']) assert(panel.includes(token), `Research Memory panel is missing ${token}`);
  assert(panel.includes('updateSuccessionArchiveBookmarkMetadata') && panel.includes('bookmarkQuery') && panel.includes('bookmarkFolder') && panel.includes('bookmarkSort'), 'bookmark folder/tag/search/sort management is not wired');
  assert(panel.includes('SUCCESSION_ARCHIVE_BOOKMARK_FOLDER_LIMIT') && panel.includes('SUCCESSION_ARCHIVE_BOOKMARK_TAG_LIMIT'), 'bookmark metadata UI does not expose bounded local persistence');
  assert(panel.includes('filteredBookmarks.length} / {memory.bookmarks.length} matching') && panel.includes('showAllBookmarks ? filteredBookmarks : filteredBookmarks.slice(0, 24)'), 'bookmark filtering/truncation disclosure is incomplete');
  assert(panel.includes('renameSuccessionWatchlist') && panel.includes('nameDirty'), 'investigation rename is not wired to stable local persistence');
  ...
  assert(css.includes('.succession-research-memory__bookmark-controls') && css.includes('.succession-research-memory__bookmark-meta'), 'bookmark management layout is missing');
  ...
  console.log(`Succession Research Memory audit passed: local recent/search/compare reducers, full bookmark folder/tag/search/sort management, investigation rename/notes/status/tags/ordering/citations, Markdown/citation/print exports, overview mount, and comparison/search handoffs are wired.`);
} finally {
  await vite.close();
}
