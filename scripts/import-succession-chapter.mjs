...
const AVAILABILITY_PATH = path.join(ROOT, 'src', 'data', 'successionChapterAvailability.generated.js');
...
Example:
  npm run import:succession-chapter -- 415 ./incoming/chapter-415
...
if (!Number.isInteger(chapter) || chapter < READER_START || chapter > MAX_CHAPTER_NUMBER) {
  throw new Error(`Chapter must be an integer from ${READER_START} through ${MAX_CHAPTER_NUMBER}.`);
}
...
const availableChapters = Object.keys(nextManifest)
  .map((value) => Number.parseInt(value, 10))
  .filter((value) => Number.isInteger(value) && value >= READER_START && value <= MAX_CHAPTER_NUMBER)
  .sort((left, right) => left - right);
const latestAvailableChapter = availableChapters.at(-1) || READER_START;
...
try {
  ...
  const generatedAvailability = `// Generated by the Succession chapter import tools.\n`
    + `// Do not edit chapter availability manually; re-run an importer instead.\n`
    + `export const authorizedSuccessionChapterNumbers = Object.freeze(${JSON.stringify(availableChapters)});\n`
    + `export const LATEST_AUTHORIZED_SUCCESSION_CHAPTER = ${latestAvailableChapter};\n`;

  await Promise.all([
    writeFile(MANIFEST_PATH, generatedContent, 'utf8'),
    writeFile(AVAILABILITY_PATH, generatedAvailability, 'utf8'),
  ]);
} catch (error) {
...
console.log(`Updated ${path.relative(ROOT, AVAILABILITY_PATH)} through Chapter ${latestAvailableChapter}.`);
