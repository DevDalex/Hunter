...
const pngDimensions = (buffer) => buffer.length >= 24 && buffer.toString('ascii', 1, 4) === 'PNG'
...
const webpDimensions = (buffer) => {
  ...
  while (offset + 8 <= buffer.length) {
    ...
    if (chunkType === 'VP8L' && dataOffset + 5 <= buffer.length && buffer[dataOffset] === 0x2f) {
      const b1 = buffer[dataOffset + 1]; const b2 = buffer[dataOffset + 2]; const b3 = buffer[dataOffset + 3]; const b4 = buffer[dataOffset + 4];
      return { width: 1 + (((b2 & 0x3f) << 8) | b1), height: 1 + (((b4 & 0x0f) << 10) | (b3 << 2) | ((b2 & 0xc0) >> 6)) };
    }
    if (chunkType === 'VP8 ' && dataOffset + 10 <= buffer.length && buffer[dataOffset + 3] === 0x9d && buffer[dataOffset + 4] === 0x01 && buffer[dataOffset + 5] === 0x2a) {
...
for (const [index, sourceName] of sourceFiles.entries()) {
  ...
  const outputName = `${String(page).padStart(3, '0')}${outputExtension}`;
  const src = `/media/succession-contest/chapters/${chapter}/${outputName}`;
  pageRecords.push({
...
const nextHistory = [...existingHistory, {
  id: `chapter-${chapter}-${Date.now()}`,
  action: previousPageCount ? 'replace-chapter' : 'import-chapter',
  chapter,
  previousPageCount,
  newPageCount: storedRecords.length,
  expectedPageCount: storedRecords.length,
  missingPages: [],
  status: 'published',
  sourceUrl: null,
