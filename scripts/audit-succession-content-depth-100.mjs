...
const [completion, reference] = await Promise.all([
  ...
  import('../src/data/succession/contentDepthExpansionReference.js'),
]);
const chapter = LATEST_DETAILED_SUCCESSION_RESEARCH_CHAPTER;
...
assert(
  chapters[0]?.scope === 'chapter-specific-pre-succession-handoff',
  'Chapter 339 must remain the chapter-specific pre-Succession handoff record',
);
