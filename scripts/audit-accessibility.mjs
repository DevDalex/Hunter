...
const assert = (condition, message) => {
  if (!condition) throw new Error(`Accessibility audit failed: ${message}`);
};
...
assert(
  ...
  'skip navigation must move focus to the main reading surface',
);
...
assert(
  contrastCss.includes('--archive-gold-ink: #6b4d00')
    && contrastCss.includes('--archive-on-dark: #f6ecee')
    && contrastCss.includes('--archive-active-contrast: #6e1825'),
  'the contrast layer must retain tested light, dark, and active-state colors',
);
...
assert(
  header.includes("event.key === 'Escape'")
    && header.includes("event.key !== 'Tab'")
    && header.includes('.header-links a, .header-actions button'),
  'the narrow-browser menu must contain focus and close with Escape',
);
assert(
  sectionTabs.includes("event.key === 'ArrowRight'")
    && sectionTabs.includes("event.key === 'Home'")
    && sectionTabs.includes("event.key === 'End'"),
  'grouped section navigation needs full keyboard movement',
);
...
for (const token of [
  'royal-map__viewport',
  'royal-map__queen-node',
  'royal-map__prince-summary',
  'royal-map__guard-mini',
  'royal-map__force-summary',
  'royal-map__controls',
  'royal-map__inspector',
]) assert(royalTreeSource.includes(token), `the accessible Royal Family relationship map is missing ${token}`);
assert(
  royalTreeSource.includes("'aria-pressed': pinned")
    && royalTreeSource.includes("aria-current={selected ? 'true' : undefined}"),
  'pin state and current prince state must remain semantically distinct',
);
assert(
  royalTreeSource.includes("'aria-controls': 'royal-map-inspector'")
    && royalTreeSource.includes('id="royal-map-inspector"'),
  'every map trigger must identify the controlled inspector',
);
...
assert(
  worldMap.includes('Skip map and open location list')
    && worldMap.includes('world-map-inspector__mobile-toggle'),
  'the retained World Atlas needs equivalent keyboard, list, and mobile-inspector access',
);
assert(
  successionApp.includes('role="status" aria-live="polite"')
