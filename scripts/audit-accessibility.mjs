...
assert(royalTreeSource.includes('aria-pressed={locked}') && royalTreeSource.includes("aria-current={active ? 'true' : undefined}") && royalTreeSource.includes("aria-current={selected ? 'true' : undefined}"), 'pin state and current household/prince state must remain semantically distinct');
assert(royalTreeSource.includes('aria-describedby={') && royalTreeSource.includes('tooltipIdFor') && royalTreeSource.includes('id={id} className="royal-board__hover-card" role="tooltip"'), 'every Royal Family preview must be programmatically associated with its trigger');
assert(royalTreeSource.includes('onFocus={activate}') && royalTreeSource.includes('onBlur={release}') && royalTreeSource.includes('onFocus={() => setActivePrinceOrder(prince.order)}'), 'Royal Family identity previews must remain keyboard accessible');
assert(royalTreeSource.includes('role="status" aria-live="polite"') && royalTree.includes('selectedPrince?.name') && !royalTree.includes("activeKey.replace(':', ' ')"), 'selection announcements must be human-readable rather than exposing internal keys');
assert(!royalTreeSource.includes('setHoveredKey') && !royalTreeSource.includes('setHoverdKey'), 'hover-only state and the misspelled blur setter must remain absent');
...
console.log('Accessibility audit passed: semantic contrast layer; legacy ARIA parent-role preservation; skip navigation; contained menus; keyboard-accessible Royal Family dossier board; distinct pinned/current states; trigger-associated previews; human-readable announcements; named household and protection regions; reduced motion and written status.');
