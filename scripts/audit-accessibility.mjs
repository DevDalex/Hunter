...
for (const token of ['royal-map__viewport', 'royal-map__queen-node', 'royal-map__prince-summary', 'royal-map__guard-mini', 'royal-map__force-summary', 'royal-map__force-member', 'royal-map__inspector']) assert(royalTreeSource.includes(token), `the accessible Royal Family relationship map is missing ${token}`);
