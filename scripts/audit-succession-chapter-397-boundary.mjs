...
try {
  ...
  assert(chapter397.voyageDay === 'Voyage Day 10', 'Chapter 397 must preserve the Voyage Day 10 framing narrative');
  assert(chapter397.chronology?.exactClockTime === null, 'Chapter 397 must not invent an exact clock time');
  assert(/historical|flashback|Meteor City/i.test(text(chapter397.chronology)), 'Chapter 397 chronology must explicitly remain historical Meteor City origin material');
  ...
  assert(projected397.length === 12, 'story-intelligence must project all 12 maintained Chapter 397 research beats alongside the dedicated event foundation');
  ...
  for (const id of dedicatedEventIds) {
    ...
    assert(event?.chronology?.day === null, `${id} must remain historical story-time rather than a literal Voyage Day 10 event`);
  }
  const founding = archive.getEntityById('event:uvogin-nominates-chrollo-spider-born');
  assert(/years-later|years later/i.test(text(founding?.chronology)), 'Spider-birth event must preserve the relative years-later coda rather than inventing an exact date');
  ...
  assert(sarasa396?.life !== 'dead', 'Sarasa’s Chapter 397 death must remain unavailable at the Chapter 396 boundary');
  ...
  for (const id of ['character:uvogin', 'character:pakunoda', 'character:shalnark']) {
    const current = archive.getCharacterStateAtChapter(id, 397);
    assert(current?.life === 'dead', `${id} must remain dead at the Chapter 397 publication-state boundary despite historical flashback appearances`);
  ...
  assert(/inference|plausible|without identifying/i.test(text(pouch)), 'Uga Forest lead must remain an evidence-based inference rather than a reconstructed perpetrator route');
  ...
  assert(!/the note says|note reads|written on the note was/i.test(String(body?.summary || '')), 'canonical event summary must not invent the note contents');
  ...
  assert(!renkoAbility396?.known, 'Renko embalming ability must not leak backward into Chapter 396 knowledge');
  assert(renkoAbility397?.known, 'Renko embalming ability must be known at Chapter 397');
  assert(renkoAbility.classification?.nenTypes?.includes('unknown'), 'Renko’s Nen category must remain unknown');
  assert(/official ability name.*unsupplied|descriptive archive label/i.test(text(renkoAbility)), 'Renko ability must remain a descriptive label rather than an invented official name');
  assert(/not resurrection|not.*resurrection|consciousness continuation/i.test(text(renkoAbility)), 'Renko ability must reject resurrection or consciousness-continuation overreach');
  ...
  assert(/Machi.*aura|aura.*Machi/i.test(text(auraEvent)), 'Machi must be recorded as perceiving aura in Chapter 397');
  ...
  assert(cemetery.parentId === 'location:meteor-city', 'Sarasa’s cemetery must remain a Meteor City location without inventing a tighter hierarchy');
  ...
  assert(/does not establish.*exact current age|exact current age.*birthday|exact.*birthday/i.test(text(threeYears)), 'three-year plan must not invent Chrollo’s exact current age or birthday');
  ...
  assert(/future plan|not.*built|unbuilt/i.test(text(network)), 'communication criminal haven must remain a plan rather than an already-built system');
  ...
  assert(/exact motive.*unresolved|no dialogue.*explaining|not.*confirmed/i.test(text(sheila)), 'Sheila’s motive must remain unresolved rather than being inferred from editorial narration');
  ...
  assert(/acting|stage/i.test(text(villain396)) && /not.*criminal|rather than criminal/i.test(text(villain396)), 'Chapter 396 Graffino boundary must remain theatrical at Chapter 396');
  ...
  assert(/ends the Chapter 396 theater-only|theater-only.*ends|rather than a stage role/i.test(text(villain397)), 'Chapter 397 must explicitly supersede the earlier theater-only boundary from this point forward');
  ...
  assert(activeAbilityNames.has('Renko’s Embalming Ability'), 'active dossier must include Renko’s Chapter 397 ability record');
  assert(!frozenAbilityNames.has('Renko’s Embalming Ability'), 'frozen through-396 dossier must remain unaware of Renko’s Chapter 397 ability');
  ...
  assert(!(dossier.successionMysteries || []).some((record) => record.chapter === 396 && /Will Sarasa encounter the kidnappers/i.test(String(record.question || ''))), 'resolved Chapter 396 Sarasa encounter cliffhanger must be retired from the active dossier');
  ...
  assert(/Nen category.*remain.*unresolved|Renko.*Nen category/i.test(note), 'source note must preserve Renko’s unknown Nen category');
  ...
  assert(/future plan|plan, not an already completed network|not an already completed network/i.test(note), 'source note must keep Chrollo’s criminal network unbuilt at Chapter 397');
