...
try {
  ...
  await record('Succession root opens the Phase 1 architecture board', desktop, async () => {
    ...
    if (subtitle.trim() !== 'Section architecture before final redesign') throw new Error(`Unexpected architecture subtitle: ${subtitle}`);
    ...
    if (storyTabs.join('|') !== 'Story|Chapters|Timeline|Events') throw new Error(`Story architecture views are incomplete: ${storyTabs.join(' | ')}`);
